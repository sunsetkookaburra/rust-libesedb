 ...
 */
int libfmapi_checksum_crc32_table_computed = 0;
      ...
      void )
{
	     ...
	     table_index++ )
	{
		...
		libfmapi_checksum_crc32_table[ table_index ] = checksum;
	}
	libfmapi_checksum_crc32_table_computed = 1;
}
...
{
	static char *function = "libfmapi_checksum_calculate_crc32";
	...
	*checksum = initial_value ^ (uint32_t) 0xffffffffUL;
	     ...
	     buffer_offset++ )
	{
		...
		*checksum = libfmapi_checksum_crc32_table[ table_index ] ^ ( *checksum >> 8 );
        }
 ...
 * Returns 1 if successful or -1 on error
 */
int libfmapi_checksum_calculate_weak_crc32(
     uint32_t *checksum,
     const uint8_t *buffer,
     size_t size,
     uint32_t initial_value,
